...
add_if() {
	local ret rc
	if ret="$(cmd ifconfig awg create name "$INTERFACE" 2>&1 >/dev/null)"; then
		return 0
	fi
	rc=$?
	...
	die "Unable to create AmneziaWG interface: $ret"
}
...
cmd_down() {
	[[ " $(awg show interfaces) " == *" $INTERFACE "* ]] || die "\`$INTERFACE' is not an AmneziaWG interface"
