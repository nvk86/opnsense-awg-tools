...
#define MAX_AWG_STRING_LEN (64U * 1024U)
...
static int kernel_get_device(struct wgdevice **device, const char *ifname)
{
	...
	*device = NULL;
	...
	if (nvlist_exists_binary(nvl_device, "h1")) {
		...
		{
			if (u32_range_from_string(&dev->init_header, (const char *)binary))
	...
	if (nvlist_exists_binary(nvl_device, "h2")) {
		...
		if (binary && size < MAX_AWG_STRING_LEN)
		{
			if (u32_range_from_string(&dev->resp_header, (const char *)binary))
	...
	if (nvlist_exists_binary(nvl_device, "h3")) {
		...
		if (binary && size < MAX_AWG_STRING_LEN)
		{
			if (u32_range_from_string(&dev->cookie_header, (const char *)binary))
	...
	if (nvlist_exists_binary(nvl_device, "h4")) {
		...
		if (binary && size < MAX_AWG_STRING_LEN)
		{
			if (u32_range_from_string(&dev->transport_header, (const char *)binary))
	...
	/* AWG 3.1 device parameters. FreeBSD kmod uses packed u16 ranges in nvlist numbers. */
	if (nvlist_exists_binary(nvl_device, "header-protection-key")) {
		...
		if (binary && size == sizeof(dev->header_protection_key)) {
			memcpy(dev->header_protection_key, binary, sizeof(dev->header_protection_key));
			dev->flags |= WGDEVICE_HAS_HEADER_PROTECTION_KEY;
		}
	}
#define AWG_GET_RANGE(_name, _field, _flag) do { \
	if (nvlist_exists_number(nvl_device, (_name))) { \
		number = nvlist_get_number(nvl_device, (_name)); \
		if (number <= UINT32_MAX) { dev->_field = (u16_range_t)number; dev->flags |= (_flag); } \
	} \
} while (0)
	AWG_GET_RANGE("content-padding-addition", content_padding_addition, WGDEVICE_HAS_CONTENT_PADDING_ADDITION);
	AWG_GET_RANGE("rekey-after-time", rekey_after_time, WGDEVICE_HAS_REKEY_AFTER_TIME);
	AWG_GET_RANGE("rekey-timeout", rekey_timeout, WGDEVICE_HAS_REKEY_TIMEOUT);
	AWG_GET_RANGE("reject-after-time", reject_after_time, WGDEVICE_HAS_REJECT_AFTER_TIME);
	AWG_GET_RANGE("keepalive-timeout", keepalive_timeout, WGDEVICE_HAS_KEEPALIVE_TIMEOUT);
	AWG_GET_RANGE("max-handshake-attempts", max_handshake_attempts, WGDEVICE_HAS_MAX_HANDSHAKE_ATTEMPTS);
#undef AWG_GET_RANGE

	/* AWG 3.1 booleans use native nvlist bools; accept 0/1 numbers for compatibility. */
	...
	} else if (nvlist_exists_number(nvl_device, "random-trailers")) {
		number = nvlist_get_number(nvl_device, "random-trailers");
		if (number <= 1) {
			dev->random_trailers = number != 0;
			dev->flags |= WGDEVICE_HAS_RANDOM_TRAILERS;
		}
	}
	...
	} else if (nvlist_exists_number(nvl_device, "disable-cookies")) {
		number = nvlist_get_number(nvl_device, "disable-cookies");
		if (number <= 1) {
			dev->disable_cookies = number != 0;
			dev->flags |= WGDEVICE_HAS_DISABLE_COOKIES;
		}
	}
	...
	for (i = 0; i < peer_count; ++i) {
		...
		if (nvlist_exists_number(nvl_peers[i], "persistent-keepalive-interval")) {
			...
			if (number <= UINT32_MAX) {
				peer->persistent_keepalive_interval = number <= UINT16_MAX ? u16_range_init((uint16_t)number, (uint16_t)number) : (u16_range_t)number;
...
static int kernel_set_device(struct wgdevice *dev)
{
	...
	for_each_wgpeer(dev, peer) {
		/* AdvancedSecurity is a legacy peer capability in the shared tools,
		 * but it has no FreeBSD nvlist peer ABI. "off" is harmless and may
		 * appear in configs exported by other platforms; reject only "on"
		 * instead of silently pretending it was applied. */
		if ((peer->flags & WGPEER_HAS_AWG) && peer->awg) {
			ret = -EOPNOTSUPP;
			goto err;
		}
		++peer_count;
	...
	if (dev->flags & WGDEVICE_HAS_H1) { const char *v = u32_range_to_string(dev->init_header); nvlist_add_binary(nvl_device, "h1", v, strlen(v) + 1); }
	if (dev->flags & WGDEVICE_HAS_H2) { const char *v = u32_range_to_string(dev->resp_header); nvlist_add_binary(nvl_device, "h2", v, strlen(v) + 1); }
	if (dev->flags & WGDEVICE_HAS_H3) { const char *v = u32_range_to_string(dev->cookie_header); nvlist_add_binary(nvl_device, "h3", v, strlen(v) + 1); }
	if (dev->flags & WGDEVICE_HAS_H4) { const char *v = u32_range_to_string(dev->transport_header); nvlist_add_binary(nvl_device, "h4", v, strlen(v) + 1); }
	...
	if (dev->flags & WGDEVICE_HAS_HEADER_PROTECTION_KEY)
		nvlist_add_binary(nvl_device, "header-protection-key", dev->header_protection_key, sizeof(dev->header_protection_key));
	if (dev->flags & WGDEVICE_HAS_CONTENT_PADDING_ADDITION)
		nvlist_add_number(nvl_device, "content-padding-addition", dev->content_padding_addition);
	if (dev->flags & WGDEVICE_HAS_REKEY_AFTER_TIME)
		nvlist_add_number(nvl_device, "rekey-after-time", dev->rekey_after_time);
	if (dev->flags & WGDEVICE_HAS_REKEY_TIMEOUT)
		nvlist_add_number(nvl_device, "rekey-timeout", dev->rekey_timeout);
	if (dev->flags & WGDEVICE_HAS_REJECT_AFTER_TIME)
		nvlist_add_number(nvl_device, "reject-after-time", dev->reject_after_time);
	if (dev->flags & WGDEVICE_HAS_KEEPALIVE_TIMEOUT)
		nvlist_add_number(nvl_device, "keepalive-timeout", dev->keepalive_timeout);
	if (dev->flags & WGDEVICE_HAS_MAX_HANDSHAKE_ATTEMPTS)
		nvlist_add_number(nvl_device, "max-handshake-attempts", dev->max_handshake_attempts);
